...
# define _EFFECT_LOCK_DATA(P, e)        \
  assert(P->effect[e]->locked==0);      \
  _aaxMutexLock(P->effect[e]->mutex);   \
  P->effect[e]->locked=1
# define _EFFECT_UNLOCK_DATA(P, e)      \
  assert(P->effect[e]->locked==1);      \
  P->effect[e]->locked=0;               \
  _aaxMutexUnLock(P->effect[e]->mutex)
# define _EFFECT_FREE_LOCK(P, e)        \
  assert(P->effect[e]->locked==1);      \
  _aaxMutexDestroy(P->effect[e]->mutex)
# define _EFFECT_GET_DATA(P, e)         \
  ((P->effect[e]->locked==1) ? P->effect[e]->data : NULL)
#else
